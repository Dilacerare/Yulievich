...
for n in range(1, const):
    for i in range(11):
        ...
        F(n)
        tt.append((time.clock() - start_time))
        nn.append(n)
...
ransac2 = LinearRegression()
ransac2.fit(np.array(nn).reshape(-1, 1), np.array(tt))
...
plt.plot(nn, ransac2.predict(np.array(nn).reshape(-1, 1)), color="red")
plt.scatter(nn, tt)
plt.show()
